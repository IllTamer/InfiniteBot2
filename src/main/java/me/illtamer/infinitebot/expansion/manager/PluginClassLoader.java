...
public class PluginClassLoader extends URLClassLoader {
    ...
    public PluginClassLoader(InfinitePluginLoader loader, ClassLoader parent, File jarFile) throws IOException, InvalidPluginException {
        ...
        try {
            ...
            throw new InvalidPluginException(StringUtil.format("When enabling jar '{0}' there got an Unknown Exception !", jar.getName()));
        }
    }
    ...
    synchronized void initialize(InfiniteExpansion infiniteExpansion) {
        Validate.notNull(infiniteExpansion, "Initializing expansion cannot be null");
        Validate.isTrue((infiniteExpansion.getClass().getClassLoader() == this), "Cannot initialize expansion outside of this class loader");
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (!entry.isDirectory() && (index = entry.getName().indexOf(".class")) != -1) {
                try {
                    ...
                    if (InfiniteExpansion.class.isAssignableFrom(clazz) || (clazz.getSuperclass() != null && "me.illtamer.infinitebot.expansion.manager.InfiniteExpansion".equals(clazz.getSuperclass().getName()))) {
